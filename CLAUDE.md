...
- `.claude-plugin/marketplace.json` — top-level marketplace manifest. Lists every plugin with `source: "./<name>"` (resolved against `metadata.pluginRoot: "./plugins"`) and its `skills`. Adding a new plugin means adding both its directory **and** an entry here.
...
- `plugins/<name>/skills/<skill>/scripts/` (and similar) — bundled helper scripts, kept inside the skill directory so they ship with `npx skills` standalone installs too (e.g. `plugins/gerrit/skills/fix-gerrit-reviews/scripts/gerrit-review.py`, `plugins/openrouter-image/skills/openrouter-image/scripts/openrouter-image`).
- `plugins/<name>/commands/<cmd>.md` — slash command definitions (Claude Code only).
- `README.md` — human-facing index of plugins and install instructions.

## Conventions when editing plugins

- **Reference bundled scripts via `${CLAUDE_PLUGIN_ROOT}`**, not `$PATH` and not `~/.claude/scripts/...`. Claude Code sets this env var to the plugin's absolute root when it loads the plugin. Scripts now live under `skills/<skill>/scripts/`, so a plugin-path reference looks like: `python3 "${CLAUDE_PLUGIN_ROOT}/skills/fix-gerrit-reviews/scripts/gerrit-review.py" query "..."`. **Skills that bundle scripts must stay installable standalone via `npx skills`** (no `CLAUDE_PLUGIN_ROOT` is set there): resolve the helper with the `if [ -n "${CLAUDE_PLUGIN_ROOT:-}" ] ... else find ...` pattern used in the gerrit/redmine/openrouter-image skills.
...
3. If the plugin needs helper scripts, put them inside the skill directory (`skills/<skill>/scripts/`) and reference them through `${CLAUDE_PLUGIN_ROOT}` (with the `npx skills` fallback pattern if the skill must work standalone).
